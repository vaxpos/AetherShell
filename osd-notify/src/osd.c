...
static void setup_brightness_monitoring(void);
...
static void refresh_mic_osd(int muted) {
    if (mic_is_muted == muted) return;

    mic_is_muted = muted;
    g_strlcpy(current_mic_text, muted ? "Muted" : "Live", sizeof(current_mic_text));
    current_osd_type = OSD_MIC;
...
void source_info_cb(pa_context *c, const pa_source_info *i, int eol, void *userdata) {
    ...
    if (mic_is_muted == -1) {
        mic_is_muted = i->mute;
        g_strlcpy(current_mic_text, i->mute ? "Muted" : "Live", sizeof(current_mic_text));
        return;
    }
...
static void refresh_brightness_osd(gboolean force_show) {
    ...
    if (!force_show && current_brightness == percentage && current_osd_type == OSD_BRIGHTNESS) {
        return;
    }
...
static void setup_brightness_monitoring(void) {
    ...
    g_io_add_watch(channel, G_IO_IN | G_IO_HUP | G_IO_ERR, on_inotify, NULL);
...
void draw_icon_microphone(cairo_t *cr, double cx, double cy, double size, int muted) {
    cairo_set_line_width(cr, 4.0);
    cairo_set_line_cap(cr, CAIRO_LINE_CAP_ROUND);
    cairo_set_line_join(cr, CAIRO_LINE_JOIN_ROUND);
    cairo_set_source_rgb(cr, 1.0, 1.0, 1.0);

    cairo_arc(cr, cx, cy - size * 0.08, size * 0.18, M_PI, 0);
    cairo_line_to(cr, cx + size * 0.18, cy + size * 0.12);
    cairo_arc(cr, cx, cy + size * 0.12, size * 0.18, 0, M_PI);
    cairo_close_path(cr);
    cairo_stroke_preserve(cr);
    cairo_fill(cr);

    cairo_move_to(cr, cx, cy + size * 0.30);
    cairo_line_to(cr, cx, cy + size * 0.48);
    cairo_move_to(cr, cx - size * 0.14, cy + size * 0.48);
    cairo_line_to(cr, cx + size * 0.14, cy + size * 0.48);
    cairo_stroke(cr);

    cairo_arc(cr, cx, cy + size * 0.10, size * 0.30, 0.1 * M_PI, 0.9 * M_PI);
    cairo_stroke(cr);

    if (muted) {
        cairo_set_source_rgb(cr, 1.0, 0.2, 0.2);
        cairo_move_to(cr, cx - size * 0.36, cy - size * 0.28);
        cairo_line_to(cr, cx + size * 0.36, cy + size * 0.44);
        cairo_stroke(cr);
    }
}
...
gboolean on_draw(GtkWidget *widget, cairo_t *cr, gpointer data) {
    ...
    // ارسم خلفية شبه شفافة حتى يظهر backdrop blur من الـ compositor
    ...
    if (current_osd_type == OSD_KEYBOARD) {
    ...
    else if (current_osd_type == OSD_VOLUME || current_osd_type == OSD_BRIGHTNESS || current_osd_type == OSD_MIC) {
        ...
        } else if (current_osd_type == OSD_BRIGHTNESS) {
            draw_icon_sun(cr, icon_cx, icon_cy, 60.0);
        } else {
            draw_icon_microphone(cr, icon_cx, icon_cy, 60.0, mic_is_muted);
        }

        if (current_osd_type == OSD_MIC) {
            cairo_select_font_face(cr, "Sans", CAIRO_FONT_SLANT_NORMAL, CAIRO_FONT_WEIGHT_BOLD);
            cairo_set_font_size(cr, 18.0);
            cairo_set_source_rgb(cr, mic_is_muted ? 1.0 : 0.0, mic_is_muted ? 0.35 : 1.0, mic_is_muted ? 0.35 : 1.0);
            cairo_text_extents_t extents;
            cairo_text_extents(cr, current_mic_text, &extents);
            cairo_move_to(cr, (width - extents.width)/2.0 - extents.x_bearing, y + h - 36.0);
            cairo_show_text(cr, current_mic_text);
        } else {
